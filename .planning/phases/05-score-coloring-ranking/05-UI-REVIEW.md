...
|---------|----------|----------|------|
| Attribution partially occluded by legend (BLOCKER licensing) | BLOCKER | **FIXED `90cc7f5`** | Lift to `--space-lg` + cap expanded credit to 60vw right-aligned so wrapped lines clear the legend column; ≥1024px E2E intersection guard |
| Collapse toggle missing focus-visible | WARNING | **FIXED `eb08ae9`** | Ink `:focus-visible` ring added |
| Score missing from pill aria-label | WARNING | **FIXED `a003ee5`** | einkunn appended to scored pill aria-label; scored-pill E2E |
